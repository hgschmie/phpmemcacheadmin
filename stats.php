...
switch($request)
{
    ...
    case 'live_stats':
        ...
        foreach($_ini->cluster($cluster) as $server_name => $server)
        {
            ...
            $actual[$server_name] = Library_Command_Factory::instance('stats_api')->stats($server['hostname'], $server['port']);

            # Calculating query time length
            $actual[$server_name]['query_time'] = max((microtime(true) - $time) * 1000, 1);
        }
